...
void sparse_solve::solve()
{
    ...
    else if ((order == 2) && (solver_type == "BCG_LUT"))
    {
        Eigen::BiCGSTAB<SpMatrx, Eigen::IncompleteLUT<double, int>> solver(EigenEqMat);
        ...
        bool is_PosDef;
        is_PosDef = is_positive_semi_definite<SpMatrx, Eigen::BiCGSTAB<SpMatrx, Eigen::IncompleteLUT<double, int>>&> (EigenEqMat*EigenEqMat.transpose(), solver);
